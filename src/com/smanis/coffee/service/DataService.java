...
public final class DataService {
    ...
    /**
     * Gets a Connection to the database. I would normally cache the Connection
     * as a Singleton, but I like to leave the app running when I suspend my
     * roasting laptop, where this causes the Connection to time out and throw
     * an error.
     *
    ...
    public ResultSet getRoastCount() throws Exception {
        String sql = this.getSql("getRoastCount");
        Statement st = this.getConnection().createStatement();
    ...
    public void updateBean(HashMap map) throws Exception {
        ...
        ps.setString(13, (String) map.get("GrindSetting"));
        ps.setString(14, (String) map.get("Comments"));
        ps.setString(15, (String) map.get("Id"));
    ...
    public String getSql(String queryName) throws Exception {
        ...
        switch (queryName) {
            ...
            case "getBeanById":
                query = "SELECT "
                        + "Id, InStock, Name, PurchaseDate, Density, GrindSetting, Altitude, ProcessMethod, Price, WeightInPounds, PricePerPound, Vendor, Origin, Variety, "
            ...
            case "getRoastCount":
                query = "SELECT COUNT(Id) as RoastCount FROM RoastLog";
                break;

            case "getRoastLogs":
                query = "SELECT "
                        + "r.Id, r.BeanId, b.Name, r.RoastStart, b.Density, r.TotalRoastTime, r.TotalDryTime, "
            ...
            case "getRoastLogsByBeanId":
                query = "SELECT Id, BeanId, RoastStart, GreenWeight, RoastedWeight, "
                        + "MoistureLossPercentage, TotalRoastTime, TotalDryTime, TotalBrowningTime, "
                        + "TotalFirstCrackTime, TotalDevelopmentTime, RoastNotes, TastingNotes, DryTime, FirstCrackEnd, EndRoast "
                        + "FROM RoastLog "
                        + "WHERE BeanId = ? "
            ...
            case "updateBean":
                query = "UPDATE Beans "
                        + "SET Name = ?, PurchaseDate = ?, Vendor = ?, ProcessMethod = ?, Price = ?, WeightInPounds = ?, "
                        + "Origin = ?, Variety = ?, Altitude = ?, DensityGrams  = ?, "
                        + "Anaerobic = ?, InStock = ?, GrindSetting = ?, Comments = ? "
                        + "Where Id = ?";
                break;
